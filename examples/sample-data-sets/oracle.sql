...
--
-- Idempotent: dropping in reverse FK order. Oracle has no DROP TABLE IF EXISTS,
-- so we use a PL/SQL wrapper that swallows ORA-00942 (table or view does not exist).

BEGIN
  FOR t IN (
    SELECT 'orders'    AS name FROM dual UNION ALL
